...
export const sendResetEmail = async (email: string, token: string) => {
  ...
  const mailOptions = {
    from: `"Shah Jalal EnterPrise" <${process.env.EMAIL_USER}>`,
    to: email,
    subject: 'Password Reset Request - Shah Jalal EnterPrise',
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
        <h2 style="color: #0d9488; text-align: center;">Shah Jalal EnterPrise</h2>
        <p>Hello,</p>
        ...
        <p style="font-size: 12px; color: #888;">Shah Jalal EnterPrise - Your Trusted Online Store</p>
      </div>
    `,
  };
